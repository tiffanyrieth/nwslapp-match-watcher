...
describe("crestUrl", () => {
	it("kickoff (no side) → the HOME club's crest; trailing slash tolerated", () => {
		...
		expect(crestUrl("https://p.example/", kickoff)).toBe("https://p.example/crest/WAS");
	});

	it("goal → the SCORING club's crest", () => {
		const [goal] = detectEvents(stored(withScores(1, 0)), withScores(1, 1)); // away scores
		expect(crestUrl("https://p.example", goal)).toBe("https://p.example/crest/ORL");
	});
	...
	it("full time → the WINNER's crest", () => {
		...
		expect(crestUrl("https://p.example", ft!)).toBe("https://p.example/crest/ORL");
	});
});
...
describe("toPayload", () => {
	...
	it("builds the contract: title+subtitle alert + mutable-content + thread-id + level + crest imageUrl", () => {
		...
		expect(payload.imageUrl).toBe(`${proxyBase}/crest/WAS`); // scoring club's square crest
	});
